...
if to == 'user' or service or is_admin1(msg) and to == "chat" or to == "channel" then
	...
	if matches[1]:lower() == 'help' and msg.to.type == 'user' or matches[1]:lower() == 'pmhelp' and is_admin1(msg) and msg.to.type ~= 'user' then
      	savelog(msg.to.id, name_log.." ["..msg.from.id.."] used pm help")
		text = "Welcome to F80!\n\nTo get a list of F80 groups use chats or chatlist for a document list of chats.\n\nTo get a new group, contact a support group:\n\nFor English support, use: join English support\n\nFor Persian support, use: join Persian support\n\nFor more information, check out our channel:\n\n@F80_CH"
     	return text
    end

	if matches[1]:lower() == 'superhelp' and is_admin1(msg)then
      	savelog(msg.to.id, name_log.." ["..msg.from.id.."] Used /superhelp")
     	return super_help()
	elseif matches[1]:lower() == 'superhelp' and to == "user" then
		local name_log = user_print_name(msg.from)
      	savelog(msg.to.id, name_log.." ["..msg.from.id.."] Used /superhelp")
     	return super_help()
    end
