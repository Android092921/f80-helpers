...
local function check_member_autorealm(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'به ریلم خود خوش امدید!')
    end
  end
end
...
local function setowner_by_reply(extra, success, result)
  ...
  data[tostring(msg.to.id)]['set_owner'] = tostring(msg.from.id)
      save_data(_config.moderation.data, data)
      savelog(msg.to.id, name_log.." ["..msg.from.id.."] set ["..msg.from.id.."] as EXADMIN")
      local text = msg.from.print_name:gsub("_", " ").." is the EXADMIN now"
...
local function modlist(msg)
  ...
  local i = 1
  local message = '\n of moderators for ' .. string.gsub(msg.to.username or msg.to.id, '_', ' ') .. ':\n'
...
if msg.to.type == 'chat' then
  if is_admin1(msg) or not is_support(msg.from.id) then-- Admin only
	  if matches[1]:lower() == 'plus' and not matches[2] then
		...
		if is_realm(msg) then
		   return 'Error: Its realm.'
		end
		savelog(msg.to.id, name_log.." ["..msg.from.id.."] added group [ "..msg.to.id.." ]")
		print("group "..msg.to.print_name.."("..msg.to.id..") added")
		return modadd(msg)
	  end
	   if matches[1]:lower() == 'plus' and matches[2] == 'realm' then
	  ...
	  if matches[1]:lower() == 'del' and matches[2] == 'realm' then
 ...
 --[[Experimental
  ...
  if msg.to.id and data[tostring(msg.to.id)] then
    ...
    if matches[1]:lower() == 'modprom' and matches[2] then
      if not is_momod(msg) then
        return
      end
      if not is_owner(msg) then
    ...
    if matches[1]:lower() == 'moddem' and not matches[2] then
	  if not is_momod(msg) then
        return
      end
      if not is_owner(msg) then
    ...
    if matches[1]:lower() == 'moddem' and matches[2] then
      if not is_momod(msg) then
        return
      end
      if not is_owner(msg) then
...
--Begin chat settings
  ...
  --End Chat muteuser
	...
	if matches[1]:lower() == "muteusers" and is_momod(msg) then
		local chat_id = msg.to.id
...
if msg.to.type == 'chat' then
    ...
    if matches[1]:lower() == 'link' then
      ...
      return "Group link ["..msg.to.title.."]:\n___________________\n__[@F80_CH]___"..group_link
    end
    if matches[1]:lower() == 'setexadmin' and matches[2] then
      if not is_admin1(msg) then
    ...
    if matches[1]:lower() == 'setexadmin' and not matches[2] then
      if not is_admin1(msg) then
        return "only for the admin!"
      end
      if type(msg.reply_id)~="nil" then
    ...
    if matches[1]:lower() == 'exadmin' then
      ...
      if not group_owner then
        return "no exadmin,ask admins in support groups to set exadmin for your group"
      end
      savelog(msg.to.id, name_log.." ["..msg.from.id.."] used /owner")
      return "Group EX Admin is ["..group_owner..']'
    end
    if matches[1]:lower() == 'setgpexadmin' then
