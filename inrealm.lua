...
function run(msg, matches)
	...
	--[[ Experimental
	if matches[1]:lower() == 'csgp' and matches[2] then
	if not is_sudo(msg) then
    ...
    if matches[1]:lower() == 'crlm' and matches[2] then
			if not is_sudo(msg) then
    ...
    local data = load_data(_config.moderation.data)
    local receiver = get_receiver(msg)
			if matches[1]:lower() == 'setabout' and matches[2] == 'group' and is_realm(msg) then
				local target = matches[3]
				local about = matches[4]
				return set_description(msg, data, target, about)
			end
			if matches[1]:lower() == 'setabout' and matches[2] == 'sgroup'and is_realm(msg) then
		...
		if matches[1]:lower() == 'settings' and matches[2] == 'group' and data[tostring(matches[3])]['settings'] then
			local target = matches[3]
			text = show_group_settingsmod(msg, target)
			return text.."\nID: "..target.."\n"
		end
		if  matches[1]:lower() == 'settings' and matches[2] == 'sgroup' and data[tostring(matches[3])]['settings'] then
			local target = matches[3]
		...
		if matches[1]:lower() == 'setgpname' and is_admin1(msg) then
    	...
    	if matches[1]:lower() == 'help' and is_realm(msg) then
		...
		if matches[1]:lower() == 'kill' and matches[2] == 'chat' and matches[3] then
			if not is_admin1(msg) then
				return
			end
		...
		if matches[1]:lower() == 'kill' and matches[2] == 'realm' and matches[3] then
			if not is_admin1(msg) then
				return
			end
			if is_realm(msg) then
		...
		if matches[1]:lower() == 'gadmin' then
		...
		if matches[1]:lower() == 'gadmindem' then
		    if not is_sudo(msg) then-- Sudo only
				return
			end
			if string.match(matches[2], '^%d+$') then
		...
		if matches[1]:lower() == 'list' and matches[2] == 'groups' then
		...
		if matches[1]:lower() == 'list' and matches[2] == 'realms' then
			if msg.to.type == 'chat' or msg.to.type == 'channel' then
				realms_list(msg)
...
return {
  patterns = {
    ...
    "^([Gg]admin) (.*)$", -- sudoers only
    "^([Gg]admindem) (.*)$", -- sudoers only
	"[#!/](support)$",
	"^[#!/](support) (.*)$",
    "^[#!/](-support) (.*)$",
    "^[#!/](list) (.*)$",
