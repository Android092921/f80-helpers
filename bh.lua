...
local support_id = msg.from.id
 if matches[1]:lower() == 'id' and msg.to.type == "chat" or msg.to.type == "user" then
    ...
    elseif matches[1]:lower() == 'id' then
      ...
      savelog(msg.to.id, name.." ["..msg.from.id.."] used /id ")
      return "آیدے گروهـِ "..msg.to.title..":"..msg.to.id.."\n"
    end
  end
  ...
  if matches[1]:lower() == 'ban' and msg.to.type == 'channel' or msg.to.type == 'chat' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "نمی تونی مقام بالا هارو بن کنی"
        end
		...
		send_large_msg(receiver, 'بــــــن شــــد')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'ban',
  ...
  if matches[1]:lower() == 'unban' and msg.to.type == 'channel' or msg.to.type == 'chat' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
...
if matches[1]:lower() == 'kick' and msg.to.type == 'channel' or msg.to.type == 'chat' then
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      if is_admin1(msg) then
	...
	elseif string.match(matches[2], '^%d+$') then
		...
		if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
			return "تو نمیتونی مقام بالا های خودتو کیک کنی:|"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
			return "مسخره کردی!"
		end
  ...
  if matches[1]:lower() == 'banall' and msg.to.type == 'channel' or msg.to.type == 'chat' and is_admin1(msg) then -- Global ban
  ...
  if matches[1]:lower() == 'unbanall' and msg.to.type == 'channel' or msg.to.type == 'chat' then -- Global unban
    local user_id = matches[2]
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return 'از همه گروه ها ازاد شــــد'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
